...
if [[ "$storage_engine" == "postgres" || "$storage_engine" == "p" ]]; then
  ...
  if [[ "$perform_migration" == "y" || "$perform_migration" == "Y" || "$perform_migration" == "yes" || "$perform_migration" == "YES" ]]; then
    ...
    log "执行数据库迁移..."
    # 这一步目前是在本地执行的，所以使用本地的 migrate 命令，连接 localhost 的 db 容器
    # 如果以后使用 app 容器内的 migrate 命令，连接字符串里的 localhost 需要修改为 $DB_HOST
    migrate -path scripts/db/migrations -database "postgres://$DB_USER:$DB_PASSWORD@localhost:$DB_PORT/$DB_NAME?sslmode=disable" up || { log "数据库迁移失败"; exit 1; }
    log "数据库迁移完成。"
...
docker-compose -f build/docker-compose.yml -f build/docker-compose-app.yml up --build app
